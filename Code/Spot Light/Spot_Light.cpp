...
int main()
{
    ...
    Shader lightCubeShader("5.4.light_cube.vs", "5.4.light_cube.fs");   // 代表光源的立方体
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 激活着色器
        lightingShader.use();
        lightingShader.setVec3("light.position", lightPos);         // 聚光源位置向量
        lightingShader.setVec3("light.direction", camera.Front);    // 聚光的方向向量
        lightingShader.setFloat("light.cutOff", glm::cos(glm::radians(12.5f)));         // 内切光角的余弦值
        lightingShader.setFloat("light.outerCutOff", glm::cos(glm::radians(17.5f)));    // 外切光角的余弦值
        lightingShader.setVec3("viewPos", camera.Position);         // 摄像机位置（观察位置）

        // 光源初始化填充，以及衰减的三个值(常数项、一次项和二次项)
