...
int main()
{
    ...
    // 创建并编译我们的着色器程序
    Shader shader("shader/4.advanced_opengl/5.1.framebuffers.vs", "shader/4.advanced_opengl/5.1.framebuffers.fs");
    Shader screenShader("shader/4.advanced_opengl/5.1.framebuffers_screen.vs", "shader/4.advanced_opengl/5.1.framebuffers_screen.fs");
    ...
    // 为深度和模板附件创建一个渲染缓冲区对象(我们不会对这些进行采样)
    unsigned int rbo;
    glGenRenderbuffers(1, &rbo);
    glBindRenderbuffer(GL_RENDERBUFFER, rbo);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, SCR_WIDTH, SCR_HEIGHT); // 对于深度和模板缓冲区使用单一的renderbuffer对象。
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, rbo); // 现在把它连接起来
    // 现在，我们实际创建了framebuffer并添加了所有附件，我们想检查它现在是否真的完成了
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 渲染指令
        // ------------
        // 绑定到framebuffer并绘制场景，就像我们通常为纹理上色一样
        glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
        glEnable(GL_DEPTH_TEST); // 启用深度测试(在渲染屏幕空间四边形时禁用)
        // 整个颜色缓冲都会被填充为glClearColor里所设置的颜色，确保我们清除了framebuffer的内容
        ...
        // 现在绑定回默认的framebuffer并使用附加的framebuffer颜色纹理绘制一个四边形平面
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        glDisable(GL_DEPTH_TEST); // 禁用深度测试，这样屏幕空间就不会因为深度测试而被丢弃。
        // 清除所有相关缓冲区
        glClearColor(1.0f, 1.0f, 1.0f, 1.0f); // 将清亮颜色设置为白色(实际上没有必要，因为我们将无法看到四边形后面)
        glClear(GL_COLOR_BUFFER_BIT);

        screenShader.use();
        glBindVertexArray(quadVAO);
        glBindTexture(GL_TEXTURE_2D, textureColorbuffer);	// 使用颜色附加纹理作为四平面的纹理
        glDrawArrays(GL_TRIANGLES, 0, 6);
